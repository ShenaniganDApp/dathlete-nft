...
import { Overrides } from "@matterlabs/hardhat-zksync-deploy";
...
async function deployFacetsZkSync(
  facets: [name: string, contract: ZkSyncContract][]
) {
  console.log("--");
  const deployed = [];
  for (const facet of facets) {
    ...
    if (!(contract instanceof ZkSyncContract)) {
      throw Error(
        `Error using facet: facet must be a Contract. Bad input: ${facet[1]}`
      );
    }
    console.log(`Using already deployed ${name}: ${contract.address}`);
    console.log("--");
    deployed.push(facet);
  }
  return deployed;
...
async function deployZkSync({
  ...
  const diamondFactory = new ZkSyncContractFactory(
    diamondArtifact.abi,
  ...
  let result;
  let initDiamondContract = {} as ZkSyncContract;
  if (!!initDiamond) {
    const [
      initDiamondName,
      initDiamondArtifact,
      initDiamondFactoryDeps,
      initDiamondConstructorArguments,
      initDiamondCustomData,
    ] = initDiamond;
    console.log(`Deploying ${initDiamondName}`);
    const initDiamondFactory = new ZkSyncContractFactory(
      initDiamondArtifact.abi,
      initDiamondArtifact.bytecode,
      owner
    );
    initDiamondContract = await initDiamondFactory.deploy(
      ...initDiamondConstructorArguments,
      {
        ..._overrides,
        customData: {
          ...initDiamondCustomData,
          factoryDeps: initDiamondFactoryDeps,
          feeToken: feeToken ?? customData?.feeToken ?? zkUtils.ETH_ADDRESS,
        },
      }
    );
    await initDiamondContract.deployed();

    result = await initDiamondContract.deployTransaction.wait();
    if (!result.status) {
      throw Error(
        `Deploying ${initDiamondName} TRANSACTION FAILED!!! -------------------------------------------`
      );
  ...
  console.log("Encoding diamondCut init function call");
  const functionCall = initDiamondContract.interface.encodeFunctionData(
    "init",
    args
  );
  // let functionCall
  ...
  const deployedDiamond = await diamondFactory.deploy(
    ...
    ...diamondConstructorArgs,
    {
      ...
      customData: {
        ...diamondCustomData,
        factoryDeps: diamondFactoryDeps,
        feeToken: feeToken ?? customData?.feeToken ?? zkUtils.ETH_ADDRESS,
      },
    }
  );
  ...
  console.log(`${diamondName} deployed: ${deployedDiamond.address}`);
  console.log(`Diamond owner: ${owner.address}`);
  ...
  // console.log(`${diamondName} diamondCut arguments:`);
  // console.log(
  //   JSON.stringify([facets, initDiamondContract.address, args], null, 4)
  // );
  result = await tx.wait();
  if (!result.status) {
    console.log(
...
exports.deployFacetsZkSync = deployFacetsZkSync;
