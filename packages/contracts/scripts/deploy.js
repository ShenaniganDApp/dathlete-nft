...
async function main(scriptName) {
  ...
  const addresses = {
    diamondAddress: dathleteDiamond.address,
    dathleteFacetAddress: dathleteFacet.address,
    challengesFacetAddress: challengesFacet.address,
    challengeTransferFacet: challengeTransferFacet.address,
    daoFacetAddress: daoFacet.address,
    shopFacetAddress: shopFacet.address,
  ...
  console.log(
    "Dathlete diamond deploy gas used:" + strDisplay(receipt.gasUsed)
  );
  totalGasUsed = totalGasUsed.add(receipt.gasUsed);
  ...
  const diamondLoupeFacet = await ethers.getContractAt(
    "DiamondLoupeFacet",
    dathleteDiamond.address
  );
  dathleteFacet = await ethers.getContractAt(
    "DathleteFacet",
    dathleteDiamond.address
  );
  ...
  if (hre.network.name === "localhost") {
    ...
    console.log(
      "Adding Challenge managers gas used::" + strDisplay(receipt.gasUsed)
    );
    totalGasUsed = totalGasUsed.add(receipt.gasUsed);

    console.log("Adding Challenge Types");
    challengesFacet = await ethers.getContractAt(
      "ChallengesFacet",
      dathleteDiamond.address
    );
    challengeTransferFacet = await ethers.getContractAt(
      "ChallengeTransferFacet",
      dathleteDiamond.address
    );
    ...
    tx = await daoFacet.addChallengeTypes(
      challengeTypes.slice(0, challengeTypes.length / 4),
      { gasLimit: gasLimit }
    );
    ...
    console.log(
      "Adding Challenge Types (1 / 4) gas used::" + strDisplay(receipt.gasUsed)
    );
    totalGasUsed = totalGasUsed.add(receipt.gasUsed);

    tx = await daoFacet.addChallengeTypes(
      challengeTypes.slice(
        challengeTypes.length / 4,
        (challengeTypes.length / 4) * 2
      ),
      { gasLimit: gasLimit }
    );
    receipt = await tx.wait();
    ...
    console.log(
      "Adding Challenge Types (2 / 4) gas used::" + strDisplay(receipt.gasUsed)
    );
    totalGasUsed = totalGasUsed.add(receipt.gasUsed);
    ...
    console.log(
      "Adding Challenge Types (3 / 4) gas used::" + strDisplay(receipt.gasUsed)
    );
    totalGasUsed = totalGasUsed.add(receipt.gasUsed);

    tx = await daoFacet.addChallengeTypes(
      challengeTypes.slice((challengeTypes.length / 4) * 3),
      { gasLimit: gasLimit }
    );
    receipt = await tx.wait();
    if (!receipt.status) {
    ...
    console.log(
      "Adding Challenge Types (4 / 4) gas used::" + strDisplay(receipt.gasUsed)
    );
    totalGasUsed = totalGasUsed.add(receipt.gasUsed);
